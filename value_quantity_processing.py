...
def process_files_and_save_prices_quantities_values(folder_path):
    folder_path = Path(folder_path)
    final_data = {}
    ...
    for year in range(2015, 2022):
        ...
        # Save the Value, Price, and Total Quantity under the respective year columns
        grouped.rename(columns={'Value': f'Value_{year}', 'Price': f'Price_{year}', 'Total Quantity': f'Total Quantity_{year}'}, inplace=True)

        # Store the result in the final_data dictionary
        final_data[year] = grouped[['HS 6', f'Price_{year}', f'Total Quantity_{year}', f'Value_{year}']]
    ...
    # Save the merged dataframe to a single Excel file
    final_df.to_excel(folder_path / "HS6_Prices_Quantities_Values_All_Years.xlsx", index=False)
...
process_files_and_save_prices_quantities_values(folder_path)